import React from 'react';
import { Link, useNavigate, useLocation } from 'react-router-dom';
import { authService } from '@shared/services';
import { Button, ProfileCard } from '@shared/components/ui';
import { appUrls } from '@shared/config';

function Layout({ children }) {
  const navigate = useNavigate();
  const location = useLocation();
  const user = authService.getCurrentUser();
  const isAuthenticated = authService.isAuthenticated();

  const isActive = (path) => {
    return location.pathname === path || location.pathname.startsWith(path + '/');
  };

  const handleLogout = () => {
    authService.logout();
    navigate('/login');
  };

  if (!isAuthenticated || user?.role !== 'SELLER') {
    return (
      <div style={{ 
        padding: '50px', 
        textAlign: 'center',
        minHeight: '100vh',
        display: 'flex',
        flexDirection: 'column',
        alignItems: 'center',
        justifyContent: 'center',
        backgroundColor: '#f5f7fa'
      }}>
        <div style={{
          backgroundColor: '#ffffff',
          padding: '40px',
          borderRadius: '12px',
          boxShadow: '0 2px 8px rgba(0, 0, 0, 0.08)',
          maxWidth: '500px'
        }}>
          <h1 style={{ marginTop: 0, color: '#111827', fontSize: '28px', fontWeight: '700' }}>Access Denied</h1>
          <p style={{ color: '#6b7280', marginBottom: '24px', fontSize: '16px' }}>
            You must be a seller to access this panel.
          </p>
          <Button onClick={() => navigate('/login')} variant="primary">
            Go to Login
          </Button>
        </div>
      </div>
    );
  }

  return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f7fa' }}>
      <nav style={{ 
        backgroundColor: '#1a1d29', 
        color: 'white', 
        padding: '12px 0',
        boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)',
        position: 'sticky',
        top: 0,
        zIndex: 1000
      }}>
        <div className="container" style={{ 
          display: 'flex', 
          justifyContent: 'space-between', 
          alignItems: 'center',
          maxWidth: '1400px',
          margin: '0 auto',
          padding: '0 24px'
        }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '24px' }}>
            <Link 
              to="/dashboard" 
              style={{ 
                color: '#20c997', 
                textDecoration: 'none', 
                fontSize: '22px', 
                fontWeight: '700',
                letterSpacing: '-0.5px'
              }}
            >
              BulkBy Seller
            </Link>
            {isAuthenticated && <ProfileCard />}
          </div>
          <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
            <Link 
              to="/dashboard" 
              style={{ 
                color: isActive('/dashboard') ? 'white' : 'rgba(255, 255, 255, 0.9)', 
                textDecoration: 'none',
                fontSize: '15px',
                fontWeight: isActive('/dashboard') ? '600' : '500',
                padding: '8px 12px',
                borderRadius: '8px',
                backgroundColor: isActive('/dashboard') ? 'rgba(255, 255, 255, 0.15)' : 'transparent',
                transition: 'all 0.2s ease'
              }}
            >
              Dashboard
            </Link>
            <Link 
              to="/products" 
              style={{ 
                color: isActive('/products') ? 'white' : 'rgba(255, 255, 255, 0.9)', 
                textDecoration: 'none',
                fontSize: '15px',
                fontWeight: isActive('/products') ? '600' : '500',
                padding: '8px 12px',
                borderRadius: '8px',
                backgroundColor: isActive('/products') ? 'rgba(255, 255, 255, 0.15)' : 'transparent',
                transition: 'all 0.2s ease'
              }}
            >
              Products
            </Link>
            <Link 
              to="/orders" 
              style={{ 
                color: isActive('/orders') ? 'white' : 'rgba(255, 255, 255, 0.9)', 
                textDecoration: 'none',
                fontSize: '15px',
                fontWeight: isActive('/orders') ? '600' : '500',
                padding: '8px 12px',
                borderRadius: '8px',
                backgroundColor: isActive('/orders') ? 'rgba(255, 255, 255, 0.15)' : 'transparent',
                transition: 'all 0.2s ease'
              }}
            >
              Orders
            </Link>
            <Link 
              to="/profile" 
              style={{ 
                color: isActive('/profile') ? 'white' : 'rgba(255, 255, 255, 0.9)', 
                textDecoration: 'none',
                fontSize: '15px',
                fontWeight: isActive('/profile') ? '600' : '500',
                padding: '8px 12px',
                borderRadius: '8px',
                backgroundColor: isActive('/profile') ? 'rgba(255, 255, 255, 0.15)' : 'transparent',
                transition: 'all 0.2s ease'
              }}
            >
              Profile
            </Link>
            <Link 
              to="/category-requests" 
              style={{ 
                color: isActive('/category-requests') ? 'white' : 'rgba(255, 255, 255, 0.9)', 
                textDecoration: 'none',
                fontSize: '15px',
                fontWeight: isActive('/category-requests') ? '600' : '500',
                padding: '8px 12px',
                borderRadius: '8px',
                backgroundColor: isActive('/category-requests') ? 'rgba(255, 255, 255, 0.15)' : 'transparent',
                transition: 'all 0.2s ease'
              }}
            >
              Category Requests
            </Link>
            <a 
              href={appUrls.userAppUrl}
              target="_blank"
              rel="noopener noreferrer"
              style={{ 
                color: 'rgba(255, 255, 255, 0.9)', 
                textDecoration: 'none',
                fontSize: '15px',
                fontWeight: '500',
                padding: '8px 12px',
                borderRadius: '8px',
                transition: 'all 0.2s ease'
              }}
            >
              View Site
            </a>
            <Button 
              onClick={handleLogout} 
              variant="danger" 
              size="sm"
              style={{
                padding: '8px 16px',
                fontSize: '14px',
                display: 'flex',
                alignItems: 'center',
                gap: '6px',
                border: '1px solid rgba(220, 53, 69, 0.3)'
              }}
            >
              <span>⏻</span>
              <span>Logout</span>
            </Button>
          </div>
        </div>
      </nav>
      <main style={{ minHeight: 'calc(100vh - 60px)' }}>{children}</main>
    </div>
  );
}

export default Layout;
